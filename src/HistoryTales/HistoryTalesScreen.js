...
const HistoryTalesScreen = () => {
    ...
    try {
      ...
      snapshot.forEach((doc) => {
        ...
        for (let i = 0; i < categories.length; i++) {
          if (
            selectedTypeKeywords.includes(categories[i]) ||
            selectedKeyword === categories[i]
          ) {
    ...
    for (const document of snapshot.docs) {
      ...
      if (data.videoId === video.videoId) {
        const commentsSnapshot = await getDocs(
          query(
            collection(firestore, `Videos/${document.id}/comments`),
  ...
  const deleteComment = async (commentDocId) => {
    ...
    for (const document of snapshot.docs) {
      ...
      if (data.videoId === selectedVideo.videoId) {
        try {
          await deleteDoc(
            doc(firestore, `Videos/${document.id}/comments`, commentDocId)
          );
          const comments = await fetchComments(selectedVideo);
          setComments(comments);
          setComment('');
      ...
      snapshot.forEach((document) => {
        ...
        if (data.videoId === video.videoId) {
          ...
          if (currentLikes.includes(userEmail)) {
            window.alert('이미 해당 영상을 성공적으로 좋아요 했습니다!');
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'row' }}>
      ...
      <Container maxWidth="xl" style={{ marginRight: '15%' }}>
        <Box sx={{ p: 2 }}>
          <Typography
            variant="h1"
            component="div"
            sx={{ fontWeight: 'bold', fontSize: 40, marginBottom: 5 }}
          >
          ...
          >
            ...
            {isLoading && (
              <CircularProgress sx={{ marginTop: 2.5, marginBottom: 2.5 }} />
            )}
            <Modal
              open={open}
              onClose={handleClose}
                        ...
                        {comment.userEmail === userEmail && (
                          <Grid item>
                            <IconButton
                              ...
                              aria-label="delete"
                              color="error"
                              onClick={() => deleteComment(comment.docId)}
                            >
                              <DeleteIcon />
                            </IconButton>
                          </Grid>
